...
void parallelProcessFilters(const vector<double>& inputData, SF_INFO& sfinfo) {
    thread iirThread(processFilter, "IIR Filter", "iir_output.wav", inputData, ref(sfinfo), iirFilter);
    thread firThread(processFilter, "FIR Filter", "fir_output.wav", inputData, ref(sfinfo), firFilter);
    thread notchThread(processFilter, "Notch Filter", "notch_output.wav", inputData, ref(sfinfo), notchFilter);
    thread bandpassThread(processFilter, "Band-pass Filter", "bandpass_output.wav", inputData, ref(sfinfo), bandPassFilter);
...
int main(int argc, char* argv[]) {
    ...
    parallelProcessFilters(originalData, sfinfo);
    auto totalEnd = high_resolution_clock::now();

    cout << "Execution Time (Total): "
         << duration_cast<milliseconds>(totalEnd - totalStart).count() + readTime << " ms" << endl;
